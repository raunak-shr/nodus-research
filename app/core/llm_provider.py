...
(anthropic | ollama | gemini) and EMBEDDING_PROVIDER (gemini | cloudflare |
ollama | hash).
"""
...
def get_llm(task: Task = "extraction") -> BaseChatModel:
    ...
    Anthropic and Gemini use a single model for every agent.
    """
...
def get_structured_llm(schema: type[BaseModel], task: Task = "extraction"):
    ...
    Gemini constrains generation to the schema natively — `GeminiChat` overrides
    this method, so it never reaches the tool-call path the others use.
    """
    return get_llm(task).with_structured_output(schema)
...
def _hash_embedder() -> Embeddings:
    logger.info(
        ...
        "Set EMBEDDING_PROVIDER=cloudflare or gemini for semantic clustering."
    )
