...
- **Being refused for room is not the same as being refused on the merits.** A ninth simultaneous request gets `too_many_requests` with a `retry_after`, not `bad_request` — as a bad request it was indistinguishable from a refusal the caller had earned, and the upload queue rendered it as a rejected file, reporting five perfectly good papers as refused for a reason that said nothing about them. A client can back off on the first and must not on the second.
...
- **A venv is not relocatable, and the error names the wrong thing.** Console scripts (`uvicorn`, `playwright`, `alembic`) carry an absolute shebang, so building a venv at one path and copying it to another leaves every one of them pointing at an interpreter that is not there. `sh` reports that as `playwright: not found` — naming the script, which exists, rather than the shebang target, which does not. The `Dockerfile` builds at the final path via `UV_PROJECT_ENVIRONMENT` instead of moving it afterwards.
  ...
  **The same bug wears a different mask locally.** Moving or renaming the checkout does it too — the `.venv` stays, and every console script installed before the move keeps pointing at the old absolute path. On Windows `uv run uvicorn …` then fails with **`Failed to canonicalize script path`**, which names neither the script nor the interpreter, and only for pip-generated launchers: `uv run python`, `uv run ruff` and anything else that is a real binary keep working, which makes it look like one package is broken rather than the venv. Confirm by reading the shebang embedded in the launcher (`.venv/Scripts/<name>.exe` carries `#!<path-to-python>` between the stub and its zip) and checking whether that path exists. `uv run python -m uvicorn app.main:app --reload` works around it — `python.exe` is a real executable, so nothing consults a stale shebang — and `uv sync --native-tls --reinstall` regenerates every launcher at the current path.
